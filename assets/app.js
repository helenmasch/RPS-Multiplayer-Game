...
connectionsRef.on("value", function(snapshot) {
  ...
  var numUser = snapshot.numChildren()
  console.log(numUser);
    if (numUser >= 2 ){
      startGame()
    }
    ...
    document.onkeyup = function (event) {
      ...
      if ((userGuess === "r") || (userGuess === "p") || (userGuess === "s")) {
        ...
        if (uniqueID.length != 0 ) {
          console.log(uniqueID.getKey())
          database.ref("/connections/" + uniqueID.getKey()).push({
            userGuess: userGuess
          })

        }
     
        $("#players").html(userGuess)
